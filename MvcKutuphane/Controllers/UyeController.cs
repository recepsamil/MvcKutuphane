...
namespace MvcKutuphane.Controllers
{
	public class UyeController : Controller
	{
		...
		*/
		...
		{
			var degerler = from u in db.TBLUYELER select u;
			...
			// Arama varsa AD'ye göre, yoksa ID'ye göre sıralama
			if (!string.IsNullOrEmpty(p))
			{
				degerler = degerler.OrderBy(x => x.AD);
			}
			else
			{
				degerler = degerler.OrderBy(x => x.ID);
			}

			var pagedListUyeler = degerler.ToPagedList(sayfa, 3);
			return View(pagedListUyeler);
		}
		...
		{
			var ktpg=db.TBLHAREKET.Where(x => x.UYE == id).ToList();
